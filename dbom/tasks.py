...
def just_save():
    ...
    if (len(cvvendor) > 0):
        ...
        try:
            ...
            pass
        info = {"web_addr": webaddr, "port": port, "username": username, "pass_wd": passwd, "token": "",
                "last_login": 0}

    cvToken = CVRestApiToken()
    cvToken.login(info)
    RestApi = CVApiOperate(cvToken)
    ...
    for job in joblist:
        ...
        if len(oldjob) > 0:
            ...
            newjob.clientname = job[3]    # ok
            newjob.idataagent = job[4]   # ok
